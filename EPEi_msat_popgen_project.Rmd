...
# install from GitHub
#devtools::install_github('ericarcher/strataG', build_vignettes = TRUE)#need internet connection. don't need to run every time, just if you think Eric's made changes and you want to update
library(strataG)
library(plyr)
rm(list = ls())
...
description <- "EiMSat_2017-02-16_noDups"
strata.file <- "Ei_strata_msats_dup_marked.csv"
genotypes.file <- "EP Ei genotypes_DupsRemoved_strataG_input.csv"
geno.data.start<-8 #make this number match the last column before genotype data
#read in data ####
Msat.Ei.geno <- readGenData(genotypes.file) #analysis set export with population column removed
Msat.Ei.geno<-subset(Msat.Ei.geno,Dups=="N")#remove previously identified/marked duplicates
#missing genotype must be "NA" (not "0")
Msat.Ei.strata <- readGenData(strata.file) #LABID and population from analysis set.
Msat.Ei.strata <- subset(Msat.Ei.strata,Dups=="N")
strata.schemes <- Msat.Ei.strata[, c("SITE","Country")]#note-here you can have additional columns, and then this makes it easy to switch between strata levels below
...
###Data QC: Check for samples missing data  
...
***(Note that we are counting the NAs, so the percent is the inversion, i.e. if more than 25% of the loci have NAs-we have data for less than 75% of the markers)***
...
Msat.Ei.geno.a<-Msat.Ei.geno[!(numNAs>((ncol(Msat.Ei.geno)-geno.data.start)*0.25)),]
```
...
t1<-table(Msat.Ei.geno.a$SITE)
t2<-table(Msat.Ei.geno.a$Country)

#run if want to remove samples where have less than X in a strata or missing data for an entire locus:
Msat.Ei.geno.short<-subset(Msat.Ei.geno.a,SITE!="Costa Careyes")
Msat.Ei.geno.short<-subset(Msat.Ei.geno.short,SITE!="La Flor")
Msat.Ei.geno.short<-subset(Msat.Ei.geno.short,SITE!="Los Cobanos")
Msat.Ei.geno.short<-subset(Msat.Ei.geno.short,SITE!="Mata Oscura")
Msat.Ei.geno.short<-subset(Msat.Ei.geno.short,SITE!="Punta Amapala")

t3<-table(Msat.Ei.geno.short$SITE)#check that it has all you want included
write.csv(t3, file = paste(description, "_counts_bystrata_afterNAremoval.csv", sep = ""))#if want to write out to output files
Msat.Ei.geno.short<-Msat.Ei.geno.short[,8:58]#remove extra columns so first one is LabID
```

###Create a diploid gtypes object####
```{r gtypes, include=TRUE}
Msat.g <- df2gtypes(Msat.Ei.geno.short, ploidy = 2, id.col=1, loc.col=2,schemes = strata.schemes)
Msat.g <- stratify(Msat.g, "SITE")
...
plot(Msat.Ei.JackHWE.influential, main = "Msat.Ei.JackHWE.influential")
dev.off()
```
