...
This takes out rows that are missing more data for than 75% of the markers. You can change the percent in the formula as you like (right now is 0.75 above).  
***(Note that we are counting the NAs, so the percent is the inversion, i.e. if more than 75% of the loci have NAs-we have data for less than 25% of the markers)***
...
Msat.Ei.geno.a<-Msat.Ei.geno[!(numNAs>((ncol(Msat.Ei.geno)-geno.data.start)*0.75)),]
```
...
table(Msat.Ei.geno.a$SITE)
table(Msat.Ei.geno.a$Country)
...
table(Msat.Ei.geno.short$SITE)#check that it has all you want included
